...
```python main.py --data-path 'data/jpg_128_128_2fps/' --model 'convolutional_LSTM' --input-width 128 --input-height 128 --nb-labels 2 --nb-lstm-units 32 --nb-conv-filters 16 --kernel-size 5 --dropout-1 0.25 --dropout-2 0.5 --nb-epochs 100 --early-stopping 15 --optimizer 'adadelta' --lr 0.001 --round-to-batch 1 --train-horses '[3,0,1,2]' --val-horses '[4]' --test-horses '[5]' --image-identifier 'jpg128_2fps_val4_t5_seq10ss10_4hl_32ubs16_flipcropshade_run5' --test-run 0 --seq-length 10 --seq-stride 10 --nb-workers 1 --batch-size 16 --nb-input-dims 5 --val-fraction 0 --data-type 'rgb' --nb-lstm-layers 4 --aug-flip 1 --aug-crop 1 --aug-light 1 ```, where
...
A data folder such as the one in the execution line specified above, `data/jpg_128_128_2fps`, (let's refer to this folder by X) should be organized as follows:
 ...
 X contains one subfolder per (horse) subject in the dataset (and once you have run, also their belonging `.csv`-files), like so:
